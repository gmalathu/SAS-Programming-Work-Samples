...
where c13_processing_date = '24OCT2014:00:00:00'dt
...
and d2_transaction_date = '24OCT2014:00:00:00'dt
...
and c14_processing_date = '24OCT2014:00:00:00'dt /* &date_dt */ /* Run daily. If today is Monday, then run for Friday & Saturday, otherwise run for yesterday.  */
;
quit;
...
where datepart(call_start_datetime) = /* intnx('day',today(),-3) >> &date_d */ '24OCT2014'd
;
quit;
...
where entry_date =  /* intnx('day',today(),-3) >> &date_d */ '24OCT2014'd
;
quit;
...
and c14_processing_date = /* intnx('dtday',datetime(),-3) &date_dt */ '24OCT2014:00:00:00'dt
;
quit;
...
,*
          ...
          when phone_type='skip2'  then 29
             ...
             when phone_type='skip9'  then 36 end as phone_type_rank
                  ,/*today()-1*/ '24OCT2014'd as join_date format=date9. /* GM060914, Since today is Monday, change today()-1 to today()-4 so it pulls back data.  */
...
where date=/*today()-1*/ '24OCT2014'd
;
quit;
...
when b.consent_code in (' ') then a.phone_type else a.phone_type end as phone_type1
...
'scell5','scell6','scell7','scell8','scell9','CELL') 
...
proc sql;
create table agg2 as select *
,case when acct_attempts > 7 then 1
when acct_attempts <= 7 and (home_attempts > 4 or cell_attempts > 2 or work_attempts > 1) then 1 
else 0 end as over

,case when acct_attempts = 7 and (home_attempts = 4 and cell_attempts = 2 and work_attempts = 1) then 1
when acct_attempts = 7 and (home_attempts <= 4 and cell_attempts <= 2 and work_attempts <= 1) then 1
when acct_attempts < 7 and (home_attempts = 4 or cell_attempts = 2 or work_attempts = 1) then 1
else 0 end as maxed
...
proc sql;
create table agg2a as select account_number
,home_attempts
,cell_attempts
,work_attempts
,other_attempts
,acct_attempts
,over
,case when over=1 and maxed=1 then 0 else maxed end as maxed
,under
from agg2
;
quit;
...
proc sql;
create table over_max_accts as select account_number
from agg2a
where over > 0
;
quit;
...
PROC EXPORT DATA= over_max_detail                                                                                                                                                                                                                                                                                                                                  
            OUTFILE="\\tsclient\C\Users\Gogie_Malathu\Documents\SAS Data\recovery_max_attempts_detail.txt"                                                                                                                                                                                                                                                                                                                                               
            DBMS=tab replace;                                                                                                                                                                                                                                                                                                                                          
     PUTNAMES=YES;                                                        
...
,b.*
from AR5_UNQ as a
left join agg2a as b on a.acct_num=b.account_number
;
quit;
...
ods pdf file='\\tsclient\C\Users\Gogie_Malathu\Documents\SAS Data\recovery_max_attempts_summary.pdf'
style=minimal;
