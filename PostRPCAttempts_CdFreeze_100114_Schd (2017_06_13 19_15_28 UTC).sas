...
and d2_transaction_date = intnx('dtday',datetime(),-4)
...
where report_date = intnx('day',today(),-4)
...
and c14_processing_date = intnx('dtday',datetime(),-4)
;
quit;
...
on a.customer_zip_code=c.zipcode
;
quit;
...
where datepart(call_start_datetime) = intnx('day',today(),-4)
;
quit;
...
where entry_date = intnx('day',today(),-4)
;
quit;
...
and substr(a.customer_zip_code,1,5)=b.zip_code
left join mike_zip_dedupe as c on substr(a.customer_zip_code,1,5)=c.zipcode
;
quit;
...
from rpc_one_a_day3
group by account_number, attempt_source, attempt_date
...
,max(est_local_time) as last_attempt_time format=time8.
...
proc sort data=rpc_one_a_day6d nodupkey out=rpc_one_a_day_6d1;
by account_number est_local_time
...
PROC EXPORT DATA= rpc_one_a_day6e                                                                                                                                                                                                                                                                                                                                    
            OUTFILE="\\tsclient\C\Users\Gogie_Malathu\Documents\SAS Data\dialer\tcpa_compliance_rptg\rpc_one_a_day6e.txt"                                                                                                                                                                                                                                                                                                                                              
            DBMS=tab replace;                                                                                                                                                                                                                                                                                                                                          
     PUTNAMES=YES;                                                        
